...
public class WatchMore {
    ...
    private static final String CONNECTSTRING = "8.129.96.243:2181";
    ...
    private boolean triggerValue(String path) throws KeeperException, InterruptedException {
        ...
        byte[] byteArray = zooKeeper.getData(path,new Watcher() {
            ...
            public void process(WatchedEvent event)
            {
                try
                {
                    System.out.println("getData triggerValue");
                    triggerValue(path);
    ...
    public static void main(String[] args) throws IOException, KeeperException, InterruptedException {
        ...
        if(watchMore.getZooKeeper().exists(PATH,false) == null){
            ...
            String zNode = watchMore.getZNode(PATH);
            System.out.println("node 1="+zNode);

            watchMore.getZooKeeper().setData(PATH,"bb".getBytes(),-1);
            zNode = watchMore.getZNode(PATH);
            System.out.println("node 2="+zNode);
        }

        watchMore.getZooKeeper().setData(PATH,"cc".getBytes(),-1);
        String zNode = watchMore.getZNode(PATH);
        System.out.println("node 3="+zNode);
        watchMore.getZooKeeper().setData(PATH,"dd".getBytes(),-1);
        zNode = watchMore.getZNode(PATH);
        System.out.println("node 4="+zNode);
    }
}